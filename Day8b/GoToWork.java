...
public class GoToWork implements Runnable {
    ...
    public GoToWork(Day8b dispatcher, char[][] square, char aChar) {
    ...
    private void writeBuffer(int valueX, int shiftX, int valueY,int shiftY) {

        int tmpX = valueX + shiftX;
        int tmpY = valueY + shiftY;

        while (!testRange.test(tmpX, tmpY)) {

            // Always save position
            buffer.add(tmpY * xMax + tmpX);
